...
import {
  getAdminBrandStoryList,
  createBrandStory,
  updateBrandStory,
  deleteBrandStory,
} from '../../api/brandStory.js'
import { uploadImage } from '../../api/swiper.js'

const typeOptions = [
  { value: 'image', label: '图片区块' },
  { value: 'video', label: '视频区块' },
]
...
async function fetchList() {
  loading.value = true
  try {
    const res = await getAdminBrandStoryList()
    if (res.code === 0) {
      list.value = res.data
    }
  } finally {
...
async function handleSubmit() {
  ...
  try {
    if (editingItem.value) {
      await updateBrandStory(editingItem.value.id, form.value)
    } else {
      await createBrandStory(form.value)
    }
...
async function handleDelete(item) {
  if (!confirm('确定删除这个区块吗？')) return
  try {
    await deleteBrandStory(item.id)
    fetchList()
  } catch (e) {
...
async function moveItem(item, direction) {
  ...
  await updateBrandStory(item.id, { ...item, sortOrder: newSort })
  await updateBrandStory(target.id, { ...target, sortOrder: oldSort })
  fetchList()
}
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="(item, idx) in list" :key="item.id">
            ...
            <td>
              <span v-if="item.videoUrl" class="has-video">已配置</span>
              <span v-else class="empty">-</span>
            </td>
            <td>
    ...
    <div v-if="dialogVisible" class="dialog-mask" @click.self="dialogVisible = false">
      <div class="dialog">
        ...
        <div class="dialog-body">
          ...
          <div v-if="form.type === 'video'" class="form-row">
            <label>视频文件</label>
            <div class="upload-area">
              <span v-if="form.videoUrl" class="video-ready">已上传视频</span>
              <label class="upload-btn">
                <input type="file" accept="video/mp4,video/webm,video/quicktime,video/ogg" @change="(e) => handleUpload('videoUrl', e)" />
                {{ form.videoUrl ? '重新上传' : '点击上传视频' }}
              </label>
            </div>
          </div>
        </div>
